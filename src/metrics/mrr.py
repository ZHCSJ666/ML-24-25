...
class MRR(Metric):
    """Mean Reciprocal Rank (MRR)@k metric.

    In contrast with accuracy, it takes a position of correct prediction among top k into account.
    """
    ...
    def update(self, predictions: torch.Tensor, references: torch.Tensor) -> None:  # type: ignore[override]
        ...
        true_pos = torch.logical_and(
            expanded_labels == top_k_predictions, expanded_labels != self.ignore_index
        )
        # mrr depends on position of correct label in top k generated outputs
        true_pos_for_mrr = true_pos / torch.arange(
            1, true_pos.size(-1) + 1, dtype=torch.float, device=true_pos.device
        )
        mrr_top_k_list = (
            true_pos_for_mrr.max(dim=-1)[0].sum(dim=-1)
            / (references != self.ignore_index).sum(dim=1).float()
        )
