...
class SimpleTransformer(nn.Module):
    ...
    def forward(self, src: Batch, src_mask=None, tgt_mask=None):
        ...
        memory = self.transformer_encoder(src_embedded, src_key_padding_mask=~src.encoder_attention_mask.bool())
        output = self.transformer_decoder(tgt_embedded, memory, tgt_key_padding_mask=~src.decoder_attention_mask.bool())
    ...
    def generate(self, src: Batch, max_length: int = 512, num_beams: int = 5, early_stopping: bool = True) -> torch.Tensor:
        ...
        start_token = torch.tensor([[self.token_embedding.padding_idx]], device=device).repeat(batch_size, 1)
        ...
        with torch.no_grad():
            ...
            memory = self.transformer_encoder(
                self.token_embedding(src.encoder_input_ids) + self.positional_encoding[:src.encoder_input_ids.size(1), :].unsqueeze(0),
            ...
            for _ in range(max_length):
                ...
                output = self.transformer_decoder(
                    self.token_embedding(decoder_input) + self.positional_encoding[:decoder_input.size(1), :].unsqueeze(0),
                    memory,
                    tgt_key_padding_mask=~(decoder_input != self.token_embedding.padding_idx).bool()
                )
