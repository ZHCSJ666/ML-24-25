...
def test_preprocess_mods():
    preprocessor = CommitChroniclePreprocessor(diff_tokenizer=None, msg_tokenizer=None, diff_line_sep="[NL]")  # tokenizers are not relevant
    ...
    assert preprocessor._preprocess_mods([delete_mod]) == "deleted file fname[NL]" + delete_mod["diff"]
...
def test_shuffle(tmp_path):
    preprocessor = CommitChroniclePreprocessor(diff_tokenizer=None, msg_tokenizer=None)  # tokenizers are not relevant
