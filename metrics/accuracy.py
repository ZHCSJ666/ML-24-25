...
class Accuracy(Metric):
    ...
    Returns a ratio of examples where reference is present among top k predictions.
    """
    ...
    def update(self, predictions: torch.Tensor, references: torch.Tensor) -> None:  # type: ignore[override]
        ...
        acc_top_k_list = (
            true_pos.sum(dim=-1).float()
            / (references != self.ignore_index).sum(dim=1).unsqueeze(1).float()
        ).sum(dim=1)
