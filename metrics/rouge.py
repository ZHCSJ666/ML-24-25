...
class ROUGEScore(Metric):
    """Calculate ROUGE score between machine generated text and reference texts.

    As input to ``forward`` and ``update`` the metric accepts the following input:

    - ``preds`` (Sequence): An iterable of machine generated text
    - ``target`` (Sequence): An iterable of reference texts

    As output of ``forward`` and ``compute`` the metric returns the following output:

    - ``rouge`` (torch.Tensor): A tensor with the ROUGE Score
    ...
    References:
        - Your reference documentation here
    """
